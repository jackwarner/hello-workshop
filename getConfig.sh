#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`hello-workshop-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`hello-workshop-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
